...
from pyzdo_core import CouchbaseHttpClient, EnvVarProvider, RootCmd
import logging
...
def get_wi_collection_hash_for_project(cmd: RootCmd) -> str:
    ...
    for c in COLLECTIONS:
        n1ql = f"SELECT * FROM pyzdo.azdo.{c} WHERE __metadata__.project_id = '{project_id}'"
        q = {
            "ql": n1ql,
            "params": [],
        }
...
#     for c in COLLECTIONS:
#         n1ql = f"SELECT id FROM pyzdo.azdo.{c} WHERE __metadata__.project_id = '{project_id}'"
#         q = {
#             "ql": n1ql,
#             "params": [],
#         }
